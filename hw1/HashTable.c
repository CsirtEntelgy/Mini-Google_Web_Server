...
// find whether key is already existed in the list.
// if found, return true and the iter points to that node
...
static bool IsKeyFound(LinkedList *list, LLIterator *iter, HTKey_t key);
...
bool HashTable_Insert(HashTable *table,
  ...
  // HTKeyValue_t is a struct type, not a pointer
  // So we have to malloc space for this struct (ie, payload)
  // where to free? -> when push into ll, the ll will hold it
  ...
  if (LinkedList_NumElements(chain) == 0) {
    // how can we know whether push successfully without bool return?
    LinkedList_Push(chain, (LLPayload_t)pair);
    table->num_elements++;  // dont forget this
    return false;  // insert succeed and no same key found
  }

  // general case; >1 elements in linked list
  LLIterator *ll_iter = LLIterator_Allocate(chain);
  if (ll_iter == NULL) {
    // we can't use assertion here, cuz assert won't free(pair)
    free(pair);
    return false;
  }

  if (IsKeyFound(chain, ll_iter, newkeyvalue.key)) {
    // the key is already present in Hashtable
    // assign old key and old value to the oldkeyvalue
    HTKeyValue_t *oldPayload;
    LLIterator_Get(ll_iter, (LLPayload_t *)(&oldPayload));
    oldkeyvalue->key = oldPayload->key;
    oldkeyvalue->value = oldPayload->value;

    // now assign new value to the existing value
    oldPayload->value = newkeyvalue.value;
    free(pair);  // reuse the existing payload, so pair no needed
    LLIterator_Free(ll_iter);  // don't forget this
    return true;
  }

  // key not found, so simply push
  LinkedList_Push(chain, (LLPayload_t)pair);
  table->num_elements++;
...
bool HashTable_Find(HashTable *table,
  ...
  // first compute which bucket and its linked list
  ...
  // general case; non-empty list, need an iter
  LLIterator *iter = LLIterator_Allocate(chain);
  Verify333(iter != NULL);

  if (IsKeyFound(chain, iter, key)) {
    // find the key
    LLPayload_t payload;
    LLIterator_Get(iter, &payload);
    HTValue_t find_val = ((HTKeyValue_t *)payload)->value;
    // set keyvalue (deep copy)
    keyvalue->key = key;
    keyvalue->value = find_val;
...
bool HashTable_Remove(HashTable *table,
  ...
  // general case; non-empty list, needs an iter
  LLIterator *iter = LLIterator_Allocate(chain);
  Verify333(iter != NULL);
  ...
  if (IsKeyFound(chain, iter, key)) {
    ...
    // set the keyvalue (deep copy)
    LLPayload_t payload;
    LLIterator_Get(iter, &payload);
    HTValue_t value = ((HTKeyValue_t *)payload)->value;
    keyvalue->key = key;
    keyvalue->value = value;

    // update elements number in HashTable
    table->num_elements--;

    // free relevant resouces from deep to shallow
    // free payload -> free LL node -> free iter
    free(payload);
...
bool HTIterator_Next(HTIterator *iter) {
  ...
  // if invalid HT iter, return false
  if (!HTIterator_IsValid(iter)) {
    return false;
  }

  // move iter->bucket_it to the next one node
  if (LLIterator_Next(iter->bucket_it)) {
    return true;
  }

  // current list has reached the end
  for (int i = iter->bucket_idx + 1; i < iter->ht->num_buckets; i++) {
    if (LinkedList_NumElements(iter->ht->buckets[i]) > 0) {
      // modify the bucket_idx and the bucket_it
      iter->bucket_idx = i;
      LLIterator_Free(iter->bucket_it);  // iter->bucket_it points to an iter
      iter->bucket_it = LLIterator_Allocate(iter->ht->buckets[i]);
      return true;
    }
  }

  // no valid next, return false
  iter->bucket_idx = INVALID_IDX;
  return false;
}
...
static bool IsKeyFound(LinkedList *list, LLIterator *iter, HTKey_t key) {
  while (true) {
    if (LinkedList_NumElements(list) == 0) {
      // empty list
      return false;
    }

    // get the key of node the ll iter is pointing
    // payload in ll POINTS TO (key,value) struct
    // LLPayload_t is void *
    LLPayload_t payload;
    LLIterator_Get(iter, &payload);
    HTKey_t exist_key = ((HTKeyValue_t *)payload)->key;

    if (exist_key == key) return true;
    if (!LLIterator_Next(iter)) return false;
  }
}
