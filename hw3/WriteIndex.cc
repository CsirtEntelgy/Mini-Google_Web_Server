...
static int WriteHeader(FILE* f, int doctable_bytes, int memidx_bytes) {
  ...
  // Read one byte one time and fold the byte into CRC.
  uint8_t byte;
...
static int WriteHashTable(FILE* f, IndexFileOffset_t offset, HashTable* ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    ...
    int32_t num_elts = LinkedList_NumElements(bucket);
    // Write bucket record to file.
    if (WriteHTBucketRecord(f, record_pos, num_elts, bucket_pos)
        == kFailedWrite) {
      return kFailedWrite;
    }
    record_pos += sizeof(BucketRecord);
    // Write bucket if it's not empty.
    if (num_elts > 0) {
      int bucket_bytes = WriteHTBucket(f, bucket_pos, bucket, fn);
      if (WriteHTBucket(f, bucket_pos, bucket, fn) == kFailedWrite) {
        return kFailedWrite;
      }
...
static int WriteHTBucket(FILE* f, IndexFileOffset_t offset, LinkedList* li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    if (fseek(f, record_pos, SEEK_SET) != 0) {
      // HERE could have a bug journal. Forget to free LLIterator.
      LLIterator_Free(it);
      return kFailedWrite;
    }
    // Initialize a ElementPositionRecord in network byte order, then write.
    ...
    // HERE could be bug journal: using reinterpret_cast and dereference.
    HTKeyValue_t kv = *(reinterpret_cast<HTKeyValue_t*>(payload));
    // Write the kv element into the index file.
