...
HashTableReader::LookupElementPositions(HTKey_t hash_key) const {
  ...
  for (int i = 0; i < bucket_rec.chain_num_elements; i++) {
    ElementPositionRecord pos_record;
    Verify333(fread(&pos_record, sizeof(ElementPositionRecord), 1, file_) == 1);
    pos_record.ToHostFormat();
    ret_val.push_back(pos_record.position);
  }
