...
bool IsPathSafe(const string& root_dir, const string& test_file) {
  ...
  char root_dir_buf[PATH_MAX];
  char *res1 = realpath(root_dir.c_str(), root_dir_buf);
  if (res1 == nullptr)
    // root not found
    return false;

  string convert1(res1);
  vector<string> root_path;
  // Split root path
  boost::split(root_path, convert1, boost::is_any_of("/"));

  // file path
  char test_file_buf[PATH_MAX];
  char *res2 = realpath(test_file.c_str(), test_file_buf);
  if (res2 == nullptr)
    // file not found
    return false;

  string convert2(res2);
  vector<string> file_path;
  // Split file path
  boost::split(file_path, convert2, boost::is_any_of("/"));
  // Get absolute path
  string abso_dir(root_path.at(root_path.size() - 1));

  for (unsigned i = 0; i < file_path.size(); i += 1) {
    if (file_path.at(i) == abso_dir) {
      return true;
    }
  }
  return false;
