...
static void GetPortAndPath(int argc,
  ...
  // check the port number reasonable?
  *port = std::stoi(argv[1]);
  if (*port > 49151 || *port < 1024) {
    cerr << "Port number not reasonable" << endl;
    Usage(argv[0]);
  }

  // check the path a readable directory
  *path = argv[2];
  struct stat stat_buf;
  if (stat((*path).c_str(), &stat_buf) == -1 || !S_ISDIR(stat_buf.st_mode)) {
    cerr << "Path not a readable directory." << endl;
    Usage(argv[0]);
  }

  // check readable index files
  for (int i = 3; i < argc; i += 1) {
    ...
    if (file_name.length() >= 4 &&
      ...
      if (stat(file_name.c_str(), &stat_buf) == -1
        ...
        // non-readable, so not push to indices
        continue;
    }
    indices->push_back(file_name);
  }

  // no readable index file
  if (indices->size() == 0) {
    cerr << "No readable index files" << endl;
    Usage(argv[0]);
  }
}
